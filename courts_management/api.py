...
def get_dashboard_data():
    ...
    # Authentic ERPNext DocType permissions (read or select) for current user
    doctype_permissions = get_user_doctype_permissions(user)
    ...
    return {
        ...
        "doctypePermissions": doctype_permissions,
        ...
        "managementOverview": [
            {"label": "Total Stores", "doctype": "Warehouse", "value": str(len(warehouses_list)), "description": "Active stores", "tone": "blue"},
            {"label": "Total Sales", "doctype": "Sales Invoice", "value": fmt(total_sales), "rawValue": total_sales, "isCurrency": True, "description": f"{total_invoices} invoices posted", "tone": "green"},
            {"label": "Procurement Spend", "doctype": "Purchase Invoice", "value": fmt(total_purchase), "rawValue": total_purchase, "isCurrency": True, "description": f"{total_purchases_count} purchase invoices", "tone": "purple"},
            {"label": "Operating Margin", "doctype": "GL Entry", "value": fmt(total_sales - total_purchase), "rawValue": total_sales - total_purchase, "isCurrency": True, "description": "Surplus before overhead", "tone": "amber"},
            {"label": "Current Inventory", "doctype": "Bin", "value": f"{int(inventory_qty):,}", "description": f"{total_bins_count} active bins", "tone": "teal"},
            {"label": "Inventory Value", "doctype": "Bin", "value": fmt(inventory_value), "rawValue": inventory_value, "isCurrency": True, "description": "Current stock valuation", "tone": "pink"},
        ],
        "kpis": {
